...
class AppState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    return Selector2<LocationModel, WeatherModel,
      ...
      builder: (context, data, child) {
        ...
        if (data.item1 == LocationPermission.denied ||
          ...
          if (data.item1 == LocationPermission.denied) {
            _text.add(Text(
                "In order to display your weather, we need permission to check your location. Allowing location access all of the time will ensure your weather is always up to date, even when the app isn't open."));
          } else {
            _text.add(Text(
                "In order to display your weather, we need permission to check your location. You'll need to allow location access in the device settings."));
          }
          return Scaffold(
            appBar: AppBar(
        ...
        if (data.item2 != null && data.item2 != 'AU') {
          return Scaffold(
            ...
            body: Padding(
              ...
              child: Text(
                  "Thanks for trying out Tidy Weather! We're currently only available in Australia, but will be expanding to other locations soon!"),
            ),
          );
        }
        ...
        ThemeData _modifiedTheme = Theme.of(context)
